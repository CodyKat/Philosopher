...
/*   Updated: 2022/07/23 01:51:45 by jaemjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_philos(t_union *info_union, t_philo *info_philo)
{
	...
	info_union->forks = sem_open("forks", O_CREAT, 0644, \
												(int)info_union->num_of_philo);
	info_union->full_count = sem_open("full_count", O_CREAT, 0644, \
												info_union->full_count);
	if (info_union->forks == SEM_FAILED || info_union->full_count == SEM_FAILED)
		ft_error();
