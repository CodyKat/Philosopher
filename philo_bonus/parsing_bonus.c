...
/*   Updated: 2022/08/13 12:32:23 by jaemjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int	num_can_get_forks;

	if (info_union->num_of_philo % 2 == 1)
		num_can_get_forks = info_union->num_of_philo / 2 + 1;
	else
		num_can_get_forks = info_union->num_of_philo / 2;
	...
	info_union->sem_can_get_forks = sem_open("sem_can_get_forks", O_CREAT, \
													S_IRWXG, num_can_get_forks);
	if (info_union->forks_set == SEM_FAILED \
		|| info_union->sem_can_get_forks == SEM_FAILED \
